class VoxaroidChat {
  constructor() {
    ...
    this.backendUrl = "https://cryptsignal-backend.onrender.com"
    this.wsUrl = this.backendUrl.replace("https://", "wss://").replace("http://", "ws://")

    // Phone inputs for both login and signup
    this.loginPhoneInput = null
  ...
  setupEventListeners() {
    ...
    document.getElementById("backToEmailBtn").addEventListener("click", () => this.showEmailForm())
    ...
    document.getElementById("themeToggleLogin").addEventListener("click", () => this.toggleLoginTheme())
  }
  ...
  // Setup phone inputs for both forms
  setupPhoneInputs() {
  ...
  showEmailVerificationForm(email) {
    ...
    this.currentForm = "emailVerification"
  ...
  showPhoneVerificationForm(phone) {
    ...
    this.currentForm = "phoneVerification"
  ...
  async loginAsGuest() {
    try {
      ...
      // Set guest plan limitations
      this.userPlan = {
        type: "guest",
        name: "Guest Mode",
        messagesPerDay: 5, // Limited messages for guests
        features: ["Basic features only", "No data persistence"],
        expiresAt: null,
      }

      this.showChatInterface()
      this.connectWebSocket()
      this.showSuccess("Welcome! You're using Voxaroid as a guest.")
  ...
  showGuestLimitations() {
    ...
    console.log("Guest mode: Limited to 5 messages, no data persistence")
  }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.needsPhoneVerification) {
          ...
          this.showChatInterface()
          this.connectWebSocket()
          this.showSuccess("Email verified! Welcome to Voxaroid!")
        }
      } else {
  ...
  async verifyPhone() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        this.showChatInterface()
        this.connectWebSocket()
        this.showSuccess("Phone verified! Welcome to Voxaroid!")
      } else {
        this.showError(data.error || "Invalid verification code")
      }
    } catch (error) {
  ...
  skipPhoneVerification() {
    // Complete login without phone verification
    const email = document.getElementById("verificationEmail").textContent
