...
const Auth = {
  ...
  async login(email, password) {
    try {
      ...
      if (response.ok) {
        STATE.currentUser = { userId: data.userId, email: data.email, name: data.name };
        ...
        return { success: true, user: STATE.currentUser, status: response.status };
      } else {
  ...
  async signup(name, email, password, phone = null) {
    try {
      if (!Utils.isValidEmail(email)) return { success: false, error: 'Please enter a valid email address' };
      ...
      if (response.ok) {
        ...
        return { success: false, error: data.detail || 'User with this email already exists', code: 'EMAIL_EXISTS', email };
      } else {
  ...
  async verifyEmail(email, code) {
    try {
      ...
      if (response.ok) {
        ...
        return { success: false, error: data.detail || 'Verification failed', status: response.status };
      }
    } catch (error) {
  ...
  // Validate token: returns true (valid), false (invalid), or null (network/transient error)
  async validateToken() {
    try {
      ...
      if (!response.ok) {
        if (response.status === 401 || response.status === 403) return false; // explicitly invalid
        return null; // transient error, don't force logout
      }
  ...
  getCurrentUser() {
    ...
    const userId = localStorage.getItem('voxaroid_userId');
    const email = localStorage.getItem('voxaroid_userEmail');
    const name = localStorage.getItem('voxaroid_userName');
    if (userId && email) {
      STATE.currentUser = { userId, email, name };
