...
const Settings = {
    ...
    loadSettings() {
        ...
        // Ensure the global STATE and settings objects exist before use
        if (typeof STATE === 'undefined' || !STATE) {
            // This is a fallback in case STATE is not defined globally yet.
            // In a real app, ensure STATE is initialized before this module.
            window.STATE = { settings: {} };
        } else if (!STATE.settings) {
            STATE.settings = {};
        }

        // Retrieve saved settings from localStorage and sync with the STATE object
        // This ensures the STATE is aligned with what the user last saved.
        const savedTheme = localStorage.getItem('voxaroid_theme') || 'light';
        const savedFontStyle = localStorage.getItem('voxaroid_fontStyle') || 'font-sans';
        const savedFontSize = localStorage.getItem('voxaroid_fontSize') || 'text-base';

        STATE.settings.theme = STATE.settings.theme || savedTheme;
        STATE.settings.fontStyle = STATE.settings.fontStyle || savedFontStyle;
        STATE.settings.fontSize = STATE.settings.fontSize || savedFontSize;
