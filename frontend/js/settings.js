...
const Settings = {
  init() {
    // Ensure STATE.settings is hydrated from localStorage
    if (!STATE.settings) {
      STATE.settings = {
  ...
  bindEvents() {
    ...
    const fs = document.getElementById('fontStyle');
    if (fs) fs.addEventListener('change', (e) => this.changeFontStyle(e.target.value));

    const fz = document.getElementById('fontSize');
    if (fz) fz.addEventListener('change', (e) => this.changeFontSize(e.target.value));
  ...
  updateThemeToggle() {
    const span = document.getElementById('themeToggleSpan');
