...
class VoxaroidApp {
  ...
  async init() {
    ...
    try {
      ...
      // Optimistic boot: if token exists, show UI immediately; validate in background
      if (Auth.isAuthenticated()) {
        await this.showApp();
        // Validate in background; only log out if explicitly invalid
        setTimeout(async () => {
          ...
          if (valid === false) {
            ...
            // transient error; keep user signed in
  ...
  bindGlobalEvents() {
    ...
    // Close modals + click-out to close mobile sidebar
  ...
  async handleLogin() {
    ...
    if (!email || !password) return Utils.showToast('Please enter both email and password', 'error');
  ...
  showEmailVerification() {
    ...
    if (!this.verificationEmail) return Utils.showToast('No email to verify. Please sign up or enter your email first.', 'warning');

    if (!document.getElementById('verificationForm')) {
      const verificationHtml = `
        <form id="verificationForm" class="space-y-6 hidden">
          <div class="text-center mb-2">
            <h3 class="text-xl font-bold mb-1">Verify Your Email</h3>
            <p class="text-gray-600 dark:text-gray-400">We sent a code to <span id="verificationEmailText"></span></p>
          </div>
          <div>
          ...
          <button id="verifyBtn" type="submit"
                  class="w-full bg-gradient-to-r from-emerald-600 to-blue-600 text-white py-3 rounded-lg font-medium hover:from-emerald-700 hover:to-blue-700">
            Verify Email
          </button>
          <div class="text-center space-x-4">
            <button id="resendCodeBtn" type="button" class="text-blue-600 dark:text-blue-400 hover:underline text-sm">Resend code</button>
            <button id="backToLoginBtn" type="button" class="text-blue-600 dark:text-blue-400 hover:underline text-sm">Back to login</button>
          </div>
        </form>
      `;
      ...
      document.getElementById('backToLoginBtn').addEventListener('click', () => this.toggleAuthForm('login'));
    }
  ...
  async handleEmailVerification() {
    ...
    if (!code || code.length !== 6) return Utils.showToast('Please enter the 6-digit verification code', 'error');
  ...
  autoResizeTextarea(textarea) {
    ...
    const newHeight = Math.min(textarea.scrollHeight, 160); // up to 160px
  ...
  bindModalCloseEvents() {
    ...
    document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape') {
        ['settingsModal', 'subscriptionModal'].forEach((id) => {
    ...
    // Click outside main content closes mobile sidebar
    document.addEventListener('click', (e) => {
      ...
      if (isOpenMobile) {
        ...
        if (!clickedInside && !clickedToggle) sidebar.classList.add('-translate-x-full');
      }
    });
  }
