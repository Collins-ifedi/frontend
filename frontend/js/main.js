...
window.handleGoogleSignIn = async (response) => {
    if (response.credential) {
        ...
        if (result.success) {
            voxaroidAppInstance.showApp(); // Assuming voxaroidAppInstance is globally accessible
        } else {
...
class VoxaroidApp {
    ...
    setVerificationEmail(email) {
        ...
        if (this.verificationEmail) {
            localStorage.setItem('voxaroid_pending_email', this.verificationEmail);
        } else {
            localStorage.removeItem('voxaroid_pending_email');
        }
    }
    ...
    bindGlobalEvents() {
        ...
        if (startVerifyBtn) {
            startVerifyBtn.addEventListener('click', async () => {
                // Use email from login field if available
                const typed = (document.getElementById('loginEmail')?.value || '').trim();
                if (typed && Utils.isValidEmail(typed)) {
                    this.setVerificationEmail(typed);
                }
                if (!this.verificationEmail) {
                ...
                try {
                    ...
                    Utils.showToast('Could not resend code', 'error');
                }
            });
        }
            ...
            if (result.success) {
                ...
                if (result.status === 403) {
                    ...
                    try {
                        ...
                        if (!resend.success) {
                            Utils.showToast(resend.error || 'Could not resend code', 'error');
                        }
                    } catch (e) {
                        // ignore
                    }
    ...
    async handleSignup() {
        ...
        try {
            ...
            if (result.success) {
                ...
                // Email already registered — guide to verification flow
    ...
    showEmailVerification() {
            ...
            const verificationHtml = `
                 <form id="verificationForm" class="space-y-6 hidden">
                     ...
                     <div class="text-center space-x-4">
                         ...
                         <button type="button" id="backToLoginBtn" class="text-blue-600 dark:text-blue-400 hover:underline text-sm">
                             Back to login
                         </button>
                     </div>
                 </form>
        ...
        // Update the email text dynamically
    ...
    async handleEmailVerification() {
        const codeRaw = document.getElementById('verificationCode').value;
        ...
        try {
            ...
            if (result.success) {
                ...
                // Clear pending verification email and go back to login
                ...
                // Show exact backend detail (expired, invalid, not found, etc.)
    ...
    async resendVerificationCode() {
        ...
        try {
            ...
            if (result.success) {
                ...
                Utils.showToast(result.error || 'Could not resend code', 'error');
            }
        } finally {
            if (resendBtn) {
...
// Main controller: sidebar open/close, auth boot, and basic UI bindings
(function () {
  ...
  window.STATE = window.STATE || {
    ...
    isConnected: false,
    settings: {
      theme: (typeof localStorage !== 'undefined' && localStorage.getItem('voxaroid_theme')) || 'light',
      fontStyle: (typeof localStorage !== 'undefined' && localStorage.getItem('voxaroid_fontStyle')) || 'font-sans',
      fontSize: (typeof localStorage !== 'undefined' && localStorage.getItem('voxaroid_fontSize')) || 'text-base'
    }
  };
  ...
  async function boot() {
    ...
    // Optimistic boot: if token exists, show app immediately; validate in background
    if (Auth.isAuthenticated()) {
      showApp();
  ...
  function bindAuthForms() {
    ...
    if (showSignupBtn && signupForm && loginForm && showLoginBtn) {
    ...
    if (loginForm) {
      loginForm.addEventListener('submit', async (e) => {
        ...
        if (res.success) {
          ...
          Utils.showToast(res.error || 'Login failed', 'error');
        }
      });
    }
    ...
    if (signupForm) {
      signupForm.addEventListener('submit', async (e) => {
        ...
        if (res.success) {
          Utils.showToast(res.message || 'Signup successful', 'success');
          // Prompt user to verify email if your flow requires it
        } else {
          Utils.showToast(res.error || 'Signup failed', 'error');
        }
      });
    }
  }
  ...
  function bindSidebarControls() {
    ...
    // Keep sidebar visible on resize to desktop
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    let voxaroidAppInstance = new VoxaroidApp();
    await voxaroidAppInstance.init().catch(error => {
